...
export const Chat = () => {
  ...
  // Retrieve active conversation message list
  ...
  // Handle location triggers (e.g. from Dashboard search or Landing page cards)
  useEffect(() => {
  ...
  const handleInitialQuery = async (queryText, category) => {
    ...
    const userMsg = { sender: 'user', text: queryText, time: new Date().toISOString() };
  ...
  const handleSendMessage = async (text, file) => {
    ...
    const userMsg = { sender: 'user', text: userMsgText, time: new Date().toISOString() };

    setIsLoading(true);

    // If no active chat, initialize session
    if (!chatId) {
      chatId = createNewChat(text ? text.slice(0, 30) + "..." : "File Upload", "General", userMsg);
      setActiveChatId(chatId);
    } else {
  ...
  // Get the last assistant message's suggested follow-up questions
